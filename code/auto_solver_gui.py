...
class Board:
	...
	def number_possible(self, x, y, number):
		for xi in range(9):
			if self.field[xi][y] == number:
				return False
		for yi in range(9):
			if self.field[x][yi] == number:
				return False
		for xi in range((x // 3)*3, (x // 3)*3 + 3):
			for yi in range((y // 3)*3, (y // 3)*3 + 3):
				if self.field[xi][yi] == number:
					return False
		return True
		"""
		checks if the 'number' can be inserted at field['x']['y']
		"""
	...
	def smart_solver(self):
		for x in range(9):
			for y in range(9):
				if self.field[x][y] == self.emptyField:
					for number in range(1,10):
						if self.number_possible(x, y, number):
							...
							self.smart_solver()
							self.field[x][y] = self.emptyField
					return
		self.end_loop()
	"""
